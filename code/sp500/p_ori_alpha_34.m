...
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% %% 交易频率信号
        if isempty(month_info_begin)||month_info_begin ~= current_month    %以月为频率进行交易
            ...
            %% 交易准备：
              getAlpha = f_Alpha034(z_open,z_ST,current_date,3,7); %获取alpha001因子
...
%   x=1:length(final_shareNum(1,site_date_ini:end));
  ...
  for fi=2:length(x)+1
      if isempty(find(final_shareNum(1,:)==index_daily(fi,1)))
          continue;
      else
              ss_shareNum(fi-1)=find(final_shareNum(1,:)==index_daily(fi,1));
      end
  end
  ss_shareNum(find(ss_shareNum==0))=[]
  y=1:length(ss_shareNum)
    plot(y-1,final_shareNum(2,ss_shareNum))
    hold on;
