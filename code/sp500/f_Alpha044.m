function alpha =f_Alpha044(z_high,z_vol,current_date,len)
...
% % (current_site-1:site_date_st-len5);
 ...
 for i=2:length(z_high(:,1))
     if (1==isnan(z_high(i,site_date_st)))
        ...
        corr_value=corrcoef(f_rank(vol_s),high_s);
 ...
 if isempty(abs_value)
     ...
     value_rank1=f_rank(abs_value);
     
     for j=1:length(value_rank1)
        alpha(value_site(j))=-1*value_rank1(j);
     end
     alpha=-alpha+0.5;
 end