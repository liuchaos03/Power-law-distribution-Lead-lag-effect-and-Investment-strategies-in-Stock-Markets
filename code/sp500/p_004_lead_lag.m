...
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% %% 交易频率信号
        if isempty(month_info_begin)||month_info_begin ~= current_month    %以月为频率进行交易
           if  sum(length(find(~isnan(z_ST(2:end,site_date_ini+daynum)))))<100
            ...
            %% 交易准备：
              getAlpha = f_Alpha004(z_low,current_date,9); %获取alpha001因子
...
%   x=1:length(final_shareNum(1,site_date_ini:end));
  ...
  for fi=2:length(x)+1
   ...
   for fi=2:length(x)+1
    ss_shareNum(fi-1)=find(final_shareNum2(1,:)==index_daily(fi,1));
  end
  figure(1);
  plot(x,index_daily(2:end,2))
   hold on;
   plot(x-1,final_lead_lag(2,ss_shareNum)/shareNum)
    hold on
    plot(x-1,final_shareNum(2,ss_shareNum)/shareNum)
    hold on;
     plot(x-1,final_shareNum2(2,ss_shareNum)/shareNum)
